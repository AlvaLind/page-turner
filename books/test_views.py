...
class BaseTestCase(TestCase):
    ...
    def setUpTestData(cls):
        """
        Create test data (users, genres, authors and books)
        that are common for all tests
        """
        cls.user = User.objects.create_user(username='testuser',
            password='12345')
        cls.genre = Genre.objects.create(name="Test Genre")
        cls.author = Author.objects.create(name='Test Author')
        cls.book1 = Book.objects.create(title="Book 1", slug="book-1",
            genre=cls.genre, author=cls.author, published_year='2020')
        cls.book2 = Book.objects.create(title="Book 2", slug="book-2",
            genre=cls.genre, author=cls.author, published_year='2021')

    @classmethod
    def setUp(cls):
        """
        Create a client/user and simulate logged-in user for all requests
        """
        cls.client = Client()
...
class BookDetailViewTest(BaseTestCase):
    def setUp(self):
        ...
        self.book = Book.objects.create(title="Test Book", slug="test-book",
            genre=self.genre, author=self.author, published_year='2022')
        self.comment = Comment.objects.create(user=self.user, book=self.book,
            body="Test comment")
    ...
    def test_add_to_bookshelf_view(self):
        ...
        """
        response = self.client.get(reverse('add_to_bookshelf',
            args=[self.book.slug]))
        # Redirects after successful add
        self.assertEqual(response.status_code, 302)
    ...
    def test_remove_from_bookshelf_view(self):
        ...
        response = self.client.get(reverse('remove_from_bookshelf',
            args=[self.book.slug]))
        # Redirects after successful remove
        self.assertEqual(response.status_code, 302)
...
class CommentEditViewTest(BaseTestCase):
    def test_comment_edit_view(self):
        ...
        comment = Comment.objects.create(user=self.user,
            book=self.book1, body="Test comment")
        response = self.client.post(reverse('comment_edit',
            args=[self.book1.slug, comment.pk]), {'body': 'Edited comment'})
...
class CommentDeleteViewTest(BaseTestCase):
    def test_comment_delete_view(self):
        ...
        response = self.client.post(reverse('comment_delete',
            args=[self.book1.slug, comment.pk]))
