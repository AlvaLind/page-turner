...
class ProfileViewTests(TestCase):
    ...
    def setUp(self):
        ...
        self.book1 = Book.objects.create(title='Book 1', slug='book-1',
            published_year='2000')
        self.book2 = Book.objects.create(title='Book 2', slug='book-2',
            published_year='2000')
        self.bookshelf1 = Bookshelf.objects.create(user=self.user,
            book=self.book1, status='unread')
        self.bookshelf2 = Bookshelf.objects.create(user=self.user,
            book=self.book2, status='reading')
    ...
    def test_profile_view_GET(self):
        ...
        to ensure it renders the profile page with users details
        """
        ...
        self.assertIn(repr(self.bookshelf1), [repr(bookshelf) for
            bookshelf in response.context['bookshelf']])
        self.assertIn(repr(self.bookshelf2), [repr(bookshelf) for
            bookshelf in response.context['bookshelf']])
    ...
    def test_profile_view_POST_valid_form(self):
        ...
        form_data = {
            ...
            # 'profile_form' key to simulate form submission
            'profile_form': '',
        }
    ...
    def test_profile_view_POST_status_update(self):
        ...
        updated_bookshelf = get_object_or_404(Bookshelf, user=self.user,
